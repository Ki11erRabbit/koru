(library (configs nano)
  ...
  (define (nano-state nano-mode)
    (modal-state (minor-mode-data nano-mode)))
  (define (nano-state-set! nano-mode new)
    (modal-state-set! (minor-mode-data nano-mode) new))

  (define (nano-prefix nano-mode)
    (modal-prefix (minor-mode-data nano-mode)))
  (define (nano-prefix-set! nano-mode new)
    (modal-prefix-set! (minor-mode-data nano-mode) new))

  (define (nano-callback-set! nano-mode new)
    (modal-callback-set! (minor-mode-data nano-mode) new))
  (define (nano-callback-apply nano-mode)
    (modal-callback-apply (minor-mode-data nano-mode)))



  (define (nano-mode)
    (let ((nano-mode (minor-mode-create 'nano-mode nano-gain-focus nano-lose-focus)))
      (let ((nano-mode-data (modal-create 'edit 'nano-mode-change (lambda (old new) (nano-change-state nano-mode new)))))
        (minor-mode-data-set! nano-mode nano-mode-data)
        nano-mode)))

  (define (nano-gain-focus nano-mode)
    (nano-change-state nano-mode (nano-state nano-mode)))
    ...
    (command-create
      ...
      (lambda (keys)
        (let ((nano-mode (minor-mode-get 'nano-mode)))
          ...
          (command-bar-show)
          (command-bar-update (nano-prefix nano-mode))
          (nano-state-set! nano-mode 'exit-write)))
      'key-sequence))
